...
class MatchesDecoder():
    class ImportedMatch():
        ...
        fields = ['wl_status', 'date', 'kills_total', 'playtime', 'map', 
            'player_1', 'player_1_primary_weapon', 'player_1_primary_ammo_A', 'player_1_primary_ammo_B', 
            'player_1_secondary_weapon', 'player_1_secondary_ammo_A', 'player_1_secondary_ammo_B', 
            'player_1_kills', 'player_1_assists', 'player_1_deaths', 'player_1_bounty', 

            'player_2', 'player_2_primary_weapon', 'player_2_primary_ammo_A', 'player_2_primary_ammo_B', 
            'player_2_secondary_weapon', 'player_2_secondary_ammo_A', 'player_2_secondary_ammo_B', 
            'player_2_kills', 'player_2_assists', 'player_2_deaths', 'player_2_bounty',  

            'player_3', 'player_3_primary_weapon', 'player_3_primary_ammo_A', 'player_3_primary_ammo_B', 
            'player_3_secondary_weapon', 'player_3_secondary_ammo_A', 'player_3_secondary_ammo_B', 
            'player_3_kills', 'player_3_assists', 'player_3_deaths', 'player_3_bounty', 

            'fights_locations']

        players_weapon_fields = [ 
            'primary_weapon', 'primary_ammo_A', 'primary_ammo_B', 
            'secondary_weapon', 'secondary_ammo_A', 'secondary_ammo_B', 
        ]
        ...
        class IncorrectCSVValueException(Exception):
            ...
            def __init__(self, field='-field-', field_type='-field_type-', message='is invalid', super_message=None, *args: object) -> None:
                super().__init__(*args)
        ...
        def normalise(self):
            for field in self.digital_fields:
                ...
                try:
                    ...
                    if value < 0:
                        raise self.IncorrectCSVValueException(field=field, field_type='digit field', problem="can't be negative")
                    
                    if value > 50 and 'bounty' not in field:
                        raise self.IncorrectCSVValueException(field=field, field_type='digit field', problem="can't be over 50")

                    if field == 'total_kills':
                        if self.good_data['player_1_kills'] + self.good_data['player_1_kills'] + self.good_data['player_1_kills'] > value:
                            value = self.good_data['player_1_kills'] + self.good_data['player_1_kills'] + self.good_data['player_1_kills']
                            self.messages.append('Total kills corrected, was less than sum of player kills.')
            ...
            for player in self.players:
                ...
                if player == "player_1":
                    ...
                    if playername == None or playername == 'None':
                        self.match_fail(f'"{player}" is set as "None", setting all his fields to None.', fail=False)
                        self.good_data[player] = None
                    else:
                        try:
                ...
                for field in self.players_weapon_fields:
                    ...
                    try:
                        ...
                        if playername == None or playername == 'None':
                            ...
                            self.good_data[fieldname] = lookup_class.objects.get(name = self.raw_data[fieldname])
                    except (Weapon.DoesNotExist, AmmoType.DoesNotExist):
                        self.match_fail(f'Incorrect value: "{self.raw_data[fieldname]}" from column "{fieldname}", "Unknown" is used instead.', fail=False)
                    except KeyError:
                        self.match_fail(f'No column "{fieldname}" provided, "Unknown" is used instead.', fail=False)

            if not self.faulty_match:
                try:
                    self.good_data['map'] = Map.objects.get(name = self.raw_data['map'])
                except Map.DoesNotExist:
                    self.match_fail(f'Map {self.raw_data["map"]} does not existust, using "Unknown map".', fail=False)
                    self.good_data['map'] = Map.objects.get(name = 'Unknown')
                except KeyError:
                    self.match_fail(f'Field "map" is missing, using "Unknown map"', fail=False)
                    self.good_data['map'] = Map.objects.get(name = 'Unknown')
                ...
                try:
                    ...
                    for location_name in locations:
                        try: 
                            ...
                            self.match_fail(f'Compound "{location_name}" does not exist."', fail=False)
                        else:
    ...
    def __init__(self, csv_file, player_owner:Player, delimeter = ',', ) -> None:        
        reader = csv.DictReader(csv_file, delimiter=delimeter)
        self.data = []
        for match in reader:
            self.data.append(self.ImportedMatch(match, player_owner = player_owner, line_number = reader.line_num))
    ...
    def transfer(self):
        ...
        for match in self.data:
            if match.faulty_match == False:
                ...
                if match_md5 in mathes_hashed:
                    self.messages.append(f"Match {match_index} already exists. This match will be discarded.")
                else:
                    if match_md5 in allowed_matches:
                        self.messages.append(f"Duplication of {match_index} found. Second instance will be discarded.")
                    else:
        ...
        for md5, processed_match in allowed_matches.items():
            db_match = Match(
                ...
                player_1_primary_weapon = processed_match.good_data["player_1_primary_weapon"], 
                player_1_primary_ammo_A = processed_match.good_data["player_1_primary_ammo_A"], 
                player_1_primary_ammo_B = processed_match.good_data["player_1_primary_ammo_B"], 
                player_1_secondary_weapon = processed_match.good_data["player_1_secondary_weapon"], 
                player_1_secondary_ammo_A = processed_match.good_data["player_1_secondary_ammo_A"], 
                player_1_secondary_ammo_B = processed_match.good_data["player_1_secondary_ammo_B"], 
                ...
                player_2_primary_weapon = processed_match.good_data["player_2_primary_weapon"], 
                player_2_primary_ammo_A = processed_match.good_data["player_2_primary_ammo_A"], 
                player_2_primary_ammo_B = processed_match.good_data["player_2_primary_ammo_B"], 
                player_2_secondary_weapon = processed_match.good_data["player_2_secondary_weapon"], 
                player_2_secondary_ammo_A = processed_match.good_data["player_2_secondary_ammo_A"], 
                player_2_secondary_ammo_B = processed_match.good_data["player_2_secondary_ammo_B"], 
                ...
                player_3_primary_weapon = processed_match.good_data["player_3_primary_weapon"], 
                player_3_primary_ammo_A = processed_match.good_data["player_3_primary_ammo_A"], 
                player_3_primary_ammo_B = processed_match.good_data["player_3_primary_ammo_B"], 
                player_3_secondary_weapon = processed_match.good_data["player_3_secondary_weapon"], 
                player_3_secondary_ammo_A = processed_match.good_data["player_3_secondary_ammo_A"], 
                player_3_secondary_ammo_B = processed_match.good_data["player_3_secondary_ammo_B"], 
