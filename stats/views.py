...
from stats.models import AmmoType, Compound, Match, Player
...
def show_stats_table(request):
    ...
    response = render(request, "stats_list.html", {'matches': matches})
    return response

def show_match_detail(request, match_id):
    try:
        match = Match.objects.get(pk=match_id)
        response = render(request, "detailed_stats.html", {'match': match})
...
def add_match_simple(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # print('DEBUG')
            # print(form.cleaned_data['teammate_1'].split('.'))
            ...
            for teammate in raw_teammates:
                ...
                if teammate[0] == 'a':
                    ...
                    teammates.append(userclass.player_username) #this should return player class of this user
