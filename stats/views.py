...
def show_stats_table(request):
    ...
    if not request.user.is_staff:
        ...
        if user['has_aka']:
            ...
            look_for_user = get(username=user['username']).username_of_player
...
def sample(request):
    # SAMPLE VIEW PLEASE IGNORE USED FOR DEBUG ONLY
    # TODO DELETE THIS VIEW
    user = AuxClass.credentials_to_dict(request)

    # aaa = Player.objects.get(also_known_as = "None")
    em = Match.objects.get(pk=12)
    matches_hashed = [match.get_md5() for match in Match.objects.all()]

    print(matches_hashed)
